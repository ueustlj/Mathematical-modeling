...
# 快速计算参数。论文终稿可加密为 2°、300 s，但耗时会明显增加。
GROUND_GRID_STEP_DEG = 5.0
TIME_STEP_S = 600.0
DURATION_S = 24.0 * 3600.0
OUTPUT_DIR = "results_q3_2_min_delay"


def build_augmented_graph(
    positions_eci_now: np.ndarray,
    time_s: float,
    ...
    positions_ecef = eci_to_ecef(positions_eci_now, time_s)
...
def main() -> None:
    ...
    snapshot_rows: list[dict[str, float | int]] = []
    all_finite_delays: list[np.ndarray] = []
    ...
    for time_index, time_s in enumerate(times):
        ...
        graph, visible_ids = build_augmented_graph(
            positions_eci,
            time_s,
            ground_unit_vectors,
        )
        ...
        finite_delays = pair_delays[finite_mask]

        if finite_delays.size == 0:
            snapshot_average = float("inf")
            snapshot_p95 = float("inf")
            snapshot_maximum = float("inf")
            reachable_ratio = 0.0
        else:
            snapshot_average = float(finite_delays.mean())
            snapshot_p95 = float(np.percentile(finite_delays, 95.0))
            snapshot_maximum = float(finite_delays.max())
            reachable_ratio = float(finite_delays.size / pair_delays.size)
            all_finite_delays.append(finite_delays)

            if snapshot_maximum > global_max_delay:
                local_index = int(np.argmax(np.where(finite_mask, pair_delays, -np.inf)))
                global_max_delay = snapshot_maximum
                global_worst_time_s = float(time_s)
                global_worst_source = int(upper_triangle[0][local_index])
                global_worst_destination = int(upper_triangle[1][local_index])
        ...
        snapshot_rows.append(
            {
                ...
                "average_delay_ms": snapshot_average,
                "p95_delay_ms": snapshot_p95,
                "maximum_delay_ms": snapshot_maximum,
                "reachable_pair_ratio": reachable_ratio,
                "minimum_visible_satellites": int(visible_counts.min()),
                "mean_visible_satellites": float(visible_counts.mean()),
                "maximum_visible_satellites": int(visible_counts.max()),
            }
        )

        print(
            f"Q3(2) progress {time_index + 1}/{len(times)}: "
            f"avg={snapshot_average:.3f} ms, "
            f"p95={snapshot_p95:.3f} ms, max={snapshot_maximum:.3f} ms"
        )
    ...
    if all_finite_delays:
        combined = np.concatenate(all_finite_delays)
        overall_average = float(combined.mean())
        overall_p95 = float(np.percentile(combined, 95.0))
    else:
        overall_average = float("inf")
        overall_p95 = float("inf")

    meets_30ms = bool(np.isfinite(global_max_delay) and global_max_delay <= 30.0)

    # 恢复最差点对的具体路由。
    worst_positions = satellite_positions_eci(CONFIG, global_worst_time_s)
    worst_graph, _ = build_augmented_graph(
        worst_positions,
        global_worst_time_s,
        ground_unit_vectors,
    )
    ...
    worst_isl_hops = max(0, len(worst_path) - 3)
    ...
    with open(output_dir / "q3_2_conclusion.txt", "w", encoding="utf-8") as file:
        file.write(
        ...
        file.write(f"仿真时长: {DURATION_S / 3600.0:.1f} h\n")
        file.write(f"区域平均端到端时延: {overall_average:.6f} ms\n")
        file.write(f"区域 95% 分位时延: {overall_p95:.6f} ms\n")
        file.write(f"区域最大端到端时延: {global_max_delay:.6f} ms\n")
        file.write(f"最大时延是否不超过 30 ms: {meets_30ms}\n")
        file.write(
            f"最差时刻: {global_worst_time_s:.1f} s "
            f"({global_worst_time_s / 3600.0:.4f} h)\n"
        )
        file.write(
            f"最差点 A: lat={source_lat:.6f} deg, lon={source_lon:.6f} deg\n"
        )
        file.write(
            f"最差点 B: lat={destination_lat:.6f} deg, lon={destination_lon:.6f} deg\n"
        )
        file.write(f"最差路径星间跳数: {worst_isl_hops}\n")
        file.write(
            "边权设置: 地面接入边只计光速传播时延；星间边计光速传播时延"
            "与 0.5 ms/跳星上处理时延。\n"
        )

    plt.figure(figsize=(9, 5))
    plt.plot(summary["time_min"], summary["average_delay_ms"], label="Average")
    plt.plot(summary["time_min"], summary["p95_delay_ms"], label="95th percentile")
    plt.plot(summary["time_min"], summary["maximum_delay_ms"], label="Maximum")
    plt.axhline(30.0, linestyle="--", label="30 ms requirement")
    plt.xlabel("Time / min")
    plt.ylabel("End-to-end delay / ms")
    plt.title("Regional end-to-end delay")
    plt.legend()
    plt.tight_layout()
    ...
    print(f"Average delay: {overall_average:.6f} ms")
    print(f"95th percentile: {overall_p95:.6f} ms")
    print(f"Maximum delay: {global_max_delay:.6f} ms")
    print(f"Meets 30 ms maximum-delay requirement: {meets_30ms}")
