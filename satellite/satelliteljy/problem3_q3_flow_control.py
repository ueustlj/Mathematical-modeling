...
def main() -> None:
    ...
    average_throughput_ratio = float(summary["throughput_ratio"].mean())
    average_delay = float(summary["average_delay_ms"].mean())
    ...
    with open(output_dir / "q3_3_conclusion.txt", "w", encoding="utf-8") as file:
        ...
        file.write(f"全日平均吞吐率: {average_throughput_ratio * 100:.4f}%\n")
        file.write(f"全日平均拥塞后时延: {average_delay:.6f} ms\n")
        file.write(
